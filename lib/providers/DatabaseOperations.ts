...
import { sessionKey, userKey, webPushSubscriptionKey } from "../constants.js"
import {
	generateUuid,
	getTableObjectKey,
	getTableEtagKey,
...
export async function GetAllNotifications(): Promise<Notification[]> {
	...
	try {
		var notificationsObject = (await localforage.startsWith(key)) as {
			[key: string]: DatabaseNotification
		}
	} catch (error) {
		console.log(error)
		return []
	...
	return Object.values(notificationsObject).map(notification =>
		ConvertDatabaseNotificationToNotification(notification)
	)
}
...
export async function SetTableObject(
	...
	try {
		if (!tableObject.Uuid) tableObject.Uuid = generateUuid()
			...
			let existingTableObject = (await localforage.getItem(
			...
			if (existingTableObject) {
				...
				for (let key of Object.keys(existingTableObject.Properties)) {
					...
					if (typeof existingTableObject.Properties[key] != "object")
						continue
					if (
						!(existingTableObject.Properties as TableObjectProperties)[
							key
						].local
					) {
						continue
...
export async function SetTableObjects(
...
export async function GetAllTableObjects(
	tableId: number = -1,
	...
	try {
		var tableObjectsObject = (await localforage.startsWith(key)) as {
			[key: string]: DatabaseTableObject
		}
...
export async function TableObjectExists(
	uuid: string,
	tableId?: number
): Promise<boolean> {
...
export function ConvertDatabaseTableObjectToTableObject(
	obj: DatabaseTableObject
): TableObject {
	...
	tableObject.BelongsToUser =
		obj.BelongsToUser != null ? obj.BelongsToUser : true
