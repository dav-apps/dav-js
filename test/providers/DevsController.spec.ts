...
import { GetDevByApiKey, DevResponseData, GetDev } from '../../lib/providers/DevsController';
...
describe("GetDev function", () => {
	it("should call getDev endpoint", async () => {
		...
		// Act
		let result = await GetDev(jwt) as ApiResponse<DevResponseData>;
	...
	it("should call getDev endpoint with error", async () => {
		...
		// Act
		let result = await GetDev(jwt) as ApiErrorResponse;
...
describe("GetDevByApiKey function", () => {
	...
	it("should call getDevByApiKey endpoint with error", async () => {
		// Arrange
		let auth = new Auth(devApiKey, devSecretKey, devUuid);
		let apiKey = "devapikey";
		let url = `${Dav.apiBaseUrl}/devs/dev/${apiKey}`;
